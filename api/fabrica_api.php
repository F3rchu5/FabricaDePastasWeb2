    ...
    function producto()
    {
      switch ($this->method)
      {
        ...
        case 'DELETE':
          if(count($this->args) > 0)
            return $this->modelProductos->borrarProducto($this->args[0]);
          break;
        case 'POST':
          if(isset($_POST['product'], $_POST['price'],$_POST['category'],$_FILES['imagesToUpload']))
            return $this->modelProducto->agregarProducto($_REQUEST['product'], $_REQUEST['price'],$_REQUEST['category'], $_FILES['imagesToUpload']);
          break;
        /*case 'PUT':
      ...
      function categoria()
      {
        switch ($this->method)
        {
          ...
          case 'DELETE':
            if(count($this->args) > 0)
              return $this->modelCategorias->borrarCategoria($this->args[0]);
            break;
          case 'POST':
            if(isset($_POST['category'],$_FILES['imagesToUpload'])))
              return $this->modelCategorias->agregarCategoria($_REQUEST['category'], $_FILES['imagesToUpload']);
            break;
          /*case 'PUT':
            if(count($this->args) > 0) return $this->model->realizarTarea($this->args[0]);
            break;*/
